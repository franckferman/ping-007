...
## Priority 2: ICMPv6 Support

Currently hardcoded to `ip4:icmp` at three points in `internal/network/network.go`
(listen socket line 56, send socket line 548, connectivity check line 548).
IPv6-only networks and dual-stack targets are not reachable.

### Implementation
- Abstract socket creation behind `newConn(network string)` where network is `ip4:icmp` or `ip6:ipv6-icmp`
- Add `--ipv6` / `-6` flag (global or per-command) to switch to `ip6:ipv6-icmp`
- ICMPv6 checksum is mandatory and computed differently (pseudo-header includes src/dst IPv6 + next-header 58) - update `internal/network/network.go` checksum logic
- TTL equivalent in IPv6 is Hop Limit (`IPV6_UNICAST_HOPS`) - update `SetTTL`
- OS signature profiles: Linux default Hop Limit = 64, Windows = 128 (same values, different field)
- `net.ResolveIPAddr("ip6", "::")` for the listener bind address
- `setsockopt(IPPROTO_IPV6, IPV6_UNICAST_HOPS, ttl)` for Hop Limit spoofing on Linux/macOS
- Windows: `SIO_RCVALL` equivalent for IPv6 raw sockets (different IOCTL)
- Auto-detect from target address family when neither `-4` nor `-6` is given

## Priority 3: Certificate-Based Auth + Operational TTL + Forward Secrecy

### Motivation
Password-based auth (`-p`) embeds a fixed secret — no revocation, no autodestruct.
A cert model gives: hard expiry (e.g. 48h), remote revocation by destroying the private key,
and ephemeral ECDH per session so in-transit captured ICMP becomes undecryptable once
the cert is gone. Content already written to disk by the listener is not affected — it is
already in cleartext. Only trafic intercepted in transit benefits from forward secrecy.

### Implementation
- `make keygen OP=fra1-drop TTL=48h` — emit ECDSA P-256 keypair with hard Not-After
- `ping-007 listen --cert fra1-drop.crt` — listener loads public cert, rejects sessions after TTL
- `ping-007 exfil --key fra1-drop.key` — sender derives ephemeral session key via ECDH(key, listener_ephemeral_pub)
- Session key = HKDF(ECDH_shared_secret) — never stored anywhere
- Revocation: destroy `fra1-drop.key` → all past in-transit captures become undecryptable
- `internal/crypto/keyexchange.go` — KeyExchangeManager (ECDH P-256 + HKDF)
- KX handshake fragmented across standard 64B ICMP packets (same CX mechanism)
- Password mode remains default; cert mode opt-in via `--key`/`--cert`

---

## Priority 4: Elliptic Curve Diffie-Hellman Key Exchange Implementation
...
## Priority 5: Security Operations Center (SOC) Evasion Techniques
...
## Priority 6: Anti-Detection Enhancement Systems
...
## Priority 7: Windows Platform Compatibility
...
## Priority 8: Documentation and User Experience
...
## Priority 9: Quality Assurance and Testing
...
## Priority 10: Performance and Scalability Optimization
...
## Priority 11: Advanced Persistent Threat (APT) Capabilities
