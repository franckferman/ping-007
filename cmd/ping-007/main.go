...
func needsRootPrivileges(args []string) bool {
	...
	// apt --list only prints profile info, no socket needed
	if command == "apt" {
		for _, arg := range args {
			if arg == "--list" {
				return false
...
or Administrator on Windows. Exceptions: "version", "status --safe",
"analyze --passive", and "apt --list" run unprivileged.`,
...
Available profiles and their default beacon intervals:
  ...
  ping-007 apt --target 10.0.0.5 --profile equation --duration 600`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if list {
				fmt.Println("Available timing profiles:")
				fmt.Println("  lazarus   Lazarus Group (DPRK)       5 min – 1 h      AES-256-GCM   high")
				fmt.Println("  apt29     Cozy Bear (RU)             30 min – 2 h     ChaCha20      very_high")
				fmt.Println("  apt28     Fancy Bear (RU)            10 min – 30 min  XOR-CFB       high")
				fmt.Println("  equation  Equation Group (NSA/TAO)   1 day – 3 days   AES-256-GCM   nation_state")
				return nil
			}
			...
			if profile == "" {
				return fmt.Errorf("required flag \"profile\" not set")
			}
	...
	cmd.Flags().Bool("list", false, "list available timing profiles and exit")
