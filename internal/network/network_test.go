...
func TestLegitimatePayload(t *testing.T) {
	...
	testCases := []struct {
		...
		{"Small data", []byte("hello"), 56},
		{"Medium data", []byte("this is a longer test message"), 56},
		{"Exact fit", make([]byte, 48), 56}, // 48 + 8 ping pattern = 56
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// First 8 bytes should be ping pattern
			expectedPattern := []byte{0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f}
			for i, b := range expectedPattern {
				if payload[i] != b {
					t.Errorf("Ping pattern mismatch at byte %d: expected %02x, got %02x", i, b, payload[i])
				}
			}
		})
	}
