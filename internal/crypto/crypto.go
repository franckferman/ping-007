// Package crypto provides the PING-007 encryption engine. It wraps three
// independent AEAD providers — AES-256-GCM, ChaCha20-Poly1305, and a custom
// XOR-CFB+HMAC scheme — behind a single CryptoEngine that can rotate the
// active algorithm at runtime without rekeying the session.
//
// Every encrypted payload carries a 4-byte header (algorithm | version | 0x00 | 0x00)
// so the receiver can auto-detect the algorithm without out-of-band negotiation.
// Keys are derived via PBKDF2-SHA256 (100k iterations) when a shared password
// is provided; otherwise crypto/rand generates ephemeral keys per session.
package crypto
...
// NonceManager generates collision-resistant nonces using a hybrid counter+random
// approach. The 64-bit monotonic counter guarantees uniqueness within a session;
// the random suffix eliminates the deterministic prediction risk of pure counters.
type NonceManager struct {
	counter uint64
	mu      sync.Mutex
}

// NewNonceManager creates a nonce manager with a zero counter.
...
// CryptoEngine wraps multiple encryption providers and exposes a single
// Encrypt/Decrypt surface. It can rotate the active algorithm on a timer
// without interrupting an ongoing session — the 4-byte header in each
// ciphertext tells the receiver which algorithm to use for that packet.
...
// CryptoConfig controls which algorithms are available, how often to rotate,
// and the shared password used for PBKDF2 key derivation.
type CryptoConfig struct {
	Enabled          bool
	Algorithms       []string
	RotationInterval time.Duration
	DefaultAlgorithm string
	SharedPassword   string // if empty, each provider generates an ephemeral random key
}

// CryptoHeader is the 4-byte prefix prepended to every ciphertext.
// It lets the receiver auto-detect the algorithm without out-of-band negotiation.
type CryptoHeader struct {
	Algorithm uint8   // algorithm ID: 1=AES-256-GCM, 2=ChaCha20-Poly1305, 3=XOR-CFB
	Version   uint8   // protocol version (currently 1)
	Reserved  [2]byte // reserved for future use; always 0x00 0x00
}

// Algorithm byte values embedded in CryptoHeader.Algorithm.
const (
...
// ContextualData carries per-packet metadata bound into the AEAD authentication
// tag. Any tampered or replayed packet will fail authentication because the
// receiver reconstructs the same context and verifies the tag.
...
// CryptoProvider is the interface every encryption backend must satisfy.
// Encrypt/Decrypt operate on raw data; the *WithContext variants bind
// additional packet metadata into the AEAD authentication tag.
...
func NewCryptoEngine(config CryptoConfig) (*CryptoEngine, error) {
	...
	// Select starting algorithm: randomize per session when the config leaves
	// the default as "aes256" (which is the viper default). This prevents a
	// fixed first-byte (0x01) in every session's crypto header — different
	// sessions now use different algorithms, breaking cross-session PCAP correlation.
	startAlg := types.CryptoAlgorithm(config.DefaultAlgorithm)
	if startAlg == types.CryptoAES256 || startAlg == "" {
		algs := []types.CryptoAlgorithm{
			types.CryptoAES256,
		...
		startAlg = algs[n%uint64(len(algs))]
	}
	engine.activeAlgorithm = startAlg
...
// SetPassword updates the shared password on all registered crypto providers
func (e *CryptoEngine) SetPassword(password string) error {
	e.mu.Lock()
	defer e.mu.Unlock()
	for _, provider := range e.providers {
		if err := provider.SetPassword(password); err != nil {
			return fmt.Errorf("failed to set password on provider %s: %w", provider.Name(), err)
		}
	}
	return nil
}
