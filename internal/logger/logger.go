...
func (l *Logger) initialize() {
	// "NONE" level: discard all output immediately — no file, no directory, no side effects.
	// This is set by --no-banner / --quiet so even `--help` leaves no trace on disk.
	if strings.ToUpper(l.config.Level) == "NONE" {
		l.slogger = slog.New(slog.NewJSONHandler(io.Discard, &slog.HandlerOptions{
			Level: slog.Level(127), // above all levels — never logs
		}))
		return
	}
