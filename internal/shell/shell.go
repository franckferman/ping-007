...
type ShellConfig struct {
	...
	// Jitter: random delay injected before each outgoing packet (0 = disabled)
	JitterMax time.Duration
}

// randJitter returns a random duration in [0, max) using crypto/rand.
...
func (s *ShellEngine) sendCommand(session *ShellSession, cmd *types.ShellCommand) error {
	// Jitter before sending — breaks fixed inter-packet timing signatures
	if j := randJitter(s.config.JitterMax); j > 0 {
		time.Sleep(j)
	}
...
func (s *ShellEngine) receiveResponse(session *ShellSession, commandID string) (*types.ShellResponse, error) {
	...
	for time.Since(startTime) < timeout {
		...
		if s.cryptoEngine != nil {
			if dec, decErr := s.cryptoEngine.Decrypt(payload); decErr == nil {
				payload = dec
			}
		}

		// Match by parsing RESP:<cmd_id>: prefix in payload
		respStr := string(payload)
		prefix := "RESP:" + commandID + ":"
		if strings.HasPrefix(respStr, prefix) {
...
func (s *ShellEngine) InteractiveShell(sessionID, target string, jitterMax time.Duration) error {
	s.config.JitterMax = jitterMax
...
func (s *ShellEngine) getSession(sessionID string) (*ShellSession, error) {
	...
	// Check timeout. Do NOT write session.Active here — we hold only a read
	// lock. CleanupExpiredSessions() handles eviction under the write lock.
